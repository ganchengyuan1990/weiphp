...
// 		    }
	...
	function me(){
		...
		$userInfo = getUserInfo($this->mid);
		$map ['uid'] = $this->mid;
	    $nihao = M('user')-> where($map)-> find();
	    $userInfo ['score'] = $nihao ['score'];
...
// 	            $v['coupon']=M('coupon')->find($v['coupon_id']);
	...
	function score_method(){
		$config=get_addon_config('SingIn');
		$config ['content'] = "你好！<br>1.节假日系统赠送积分；<br>2.每日签到或者分享文章，得1分，连续7天签到额外赠送5分；<br>3.完成任务可加分（请关注每日推送信息）";
