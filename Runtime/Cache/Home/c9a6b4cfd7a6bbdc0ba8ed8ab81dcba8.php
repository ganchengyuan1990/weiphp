      ...
      <?php if($add_button || $del_button || $search_button || !empty($top_more_button)): ?><div class="table-bar">
        <div class="fl">
          <?php if(empty($model["extend"])): ?><div class="tools">
			...
			&nbsp;&nbsp;交易方式：
              <select name="credit_type" style="padding:3px 4px; margin-bottom:5px; width:120px" >
              	<option value="0" <?php if($_GET['credit_type']==0)echo 'selected=selected'; ?>>全部方式</option>
              	<?php if(!empty($credit_title)): if(is_array($credit_title)): $k = 0; $__LIST__ = $credit_title;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$s): $mod = ($k % 2 );++$k;?><option value="<?php echo ($k); ?>" <?php if($_GET['credit_type']==$k)echo 'selected=selected'; ?>><?php echo ($s["title"]); ?></option><?php endforeach; endif; else: echo "" ;endif; endif; ?>
              </select>
      
     &nbsp;&nbsp;  交易时间：&nbsp; <input type="datetime" placeholder="请选择交易日期" value="<?php echo ($_GET['start_ctime']); ?>" class="text date" name="start_ctime" style="width: 130px;">
            至
            <input type="datetime" placeholder="请选择交易日期" value="<?php echo ($_GET['end_ctime']); ?>" class="text date" name="end_ctime" style="width: 130px;">
         &nbsp;<a href='javascript:;' class='btn' id='ctimeSearch' style='margin-top: -9px;'>查询</a><?php endif; ?>
        </div>
        <!-- 高级搜索 -->
...
$(function(){
	...
	$("#search").click(function(){
        ...
        if(query == '' ){
        	...
        	if(addon){
        		url="<?php echo addons_url($_REQUEST ['_addons'].'://'.$_REQUEST ['_controller'].'/score_lists');?>&mdm=<?php echo ($_GET['mdm']); ?>";
        	}
        }
    ...
    $("select[name='credit_type']").change(function(){
		var url = "<?php echo addons_url($_REQUEST ['_addons'].'://'.$_REQUEST ['_controller'].'/score_lists');?>&mdm=<?php echo ($_GET["mdm"]); ?>&credit_type="+$(this).val();
    ...
    $('#ctimeSearch').click(function(){
		...
		if(start || end){
			var url="<?php echo addons_url($_REQUEST ['_addons'].'://'.$_REQUEST ['_controller'].'/score_lists');?>&mdm=<?php echo ($_GET["mdm"]); ?>&is_ctime=1&start_ctime="+start+"&end_ctime="+end;
			location.href=url;
		}
