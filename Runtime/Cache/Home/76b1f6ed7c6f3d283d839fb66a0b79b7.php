        ...
        <?php if($search_button): ?><div class="search-form fr cf">
          <div class="sleft" style="margin-right:10px">
           <input style="width:100px;" type="text" name="number" class="search-input" value="<?php echo $_GET['number'];?>" placeholder="请输入会员卡号">
            <a class="sch-btn" href="javascript:;" id="number_search" url="<?php echo U('lists?model='.$model['id'],array('mdm'=>$_GET['mdm']));?>"><i class="btn-search"></i></a> 
           </div>
           <div class="sleft" style="margin-right:10px">
           
            <input style="width:100px;" type="text" name="phone" class="search-input" value="<?php echo $_GET['phone'];?>" placeholder="请输入手机号">
            <a class="sch-btn" href="javascript:;" id="phone_search" url="<?php echo U('lists?model='.$model['id'],array('mdm'=>$_GET['mdm']));?>"><i class="btn-search"></i></a> 
            </div>
           <div class="sleft">
...
<!--                     <td><?php echo get_list_field($data,$grid,$model);?></td> -->
<!--<?php endforeach; endif; else: echo "" ;endif; ?> -->
							...
							<td><?php echo (get_name_by_status($data["status"])); ?></td>
							<td>
								<a target="_self" href="<?php echo U('edit?model='.$model['id'],array('id'=>$data['id']));?>">编辑</a>
								<a target="confirm" href="<?php echo U('del?model='.$model['id'],array('id'=>$data['id']));?>">删除</a>
								<a onClick="$.WeiPHP.openSubmitDialog('会员充值','<?php echo U('do_recharge?model='.$model['id'],array('id'=>$data['id']));?>',600,400)" href="javascript:;">会员充值</a>
								<a onClick="$.WeiPHP.openSubmitDialog('会员消费','<?php echo U('do_buy?model='.$model['id'],array('id'=>$data['id']));?>',600,560)" href="javascript:;">会员消费</a>
								<a onClick="$.WeiPHP.openSubmitDialog('手动修改积分','<?php echo U('update_score?model='.$model['id'],array('id'=>$data['id']));?>',600,400)" href="javascript:;">手动修改积分</a>
							</td>
